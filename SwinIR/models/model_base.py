...
class ModelBase():
    ...
    def adjust_lr(self, epoch, warmup_epoch):
        for scheduler in self.schedulers:
            for param_group in scheduler.optimizer.param_groups:
                lr = scheduler.get_lr()[0]
                new_lr = lr * min(epoch + 1, warmup_epoch + 1e-6) / (warmup_epoch + 1e-6)
                param_group['lr'] = new_lr

    def recover_lr(self, epoch, warmup_epoch):
        for scheduler in self.schedulers:
            for param_group in scheduler.optimizer.param_groups:
                lr = scheduler.get_lr()[0]
                new_lr = lr / min(epoch + 1, warmup_epoch + 1e-6) * (warmup_epoch + 1e-6)
