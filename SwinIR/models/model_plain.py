...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        self.dual_weight = self.opt_train['dual_weight'] if 'dual_weight' in self.opt_train else 0
    ...
    def netG_forward(self):
        self.E, self.sr2lr = self.netG(self.L)

    def netE_forward(self):
        self.E, self.sr2lr = self.netE(self.L)
    ...
    def optimize_parameters(self, current_step, teacher=None):
        ...
        if self.sr2lr is not None:
            dual_loss = self.G_lossfn(self.sr2lr, self.L)
            G_loss = G_loss + self.dual_weight * dual_loss
        if teacher is not None:
            teacher.netG.eval()
            teacher_result, _ = teacher.netG(self.L)
            G_loss = G_loss + self.G_lossfn_weight * self.G_lossfn(self.E, teacher_result)
        G_loss.backward()
        ...
        if self.sr2lr is not None:
            self.log_dict['dual_loss'] = dual_loss.item()
