...
def model_test(model, val_loader, opt, logger):
    avg_psnr = 0.0
    idx = 0
    ...
    logger.info(
        "Test before training psnr: {:.6f}".format(avg_psnr)
    )
...
def main():
    ...
    opt = option.parse(args.opt, is_train=True, exp_root=args.exp_root)
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt["logger"]["print_freq"] == 0 and rank == 0:
                ...
                message = "<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}> ".format(
                    epoch, current_step, model.get_current_learning_rate() * lr_scale
                )
            ...
            if current_step % opt["train"]["val_freq"] == 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    save_img_path = os.path.join(
                        opt["path"]["val_images"], "{:s}_{:d}.png".format(img_name, current_step)
                    )
                ...
                if avg_psnr > 20:
                    ...
                    print(f"# Validation crashed on rank {rank}, use previous state_dict...\n")

                    tmp_opt = opt.copy()
                    tmp_ckpt_path = os.path.join(
                        opt["path"]["models"], "{}_G.pth".format(best_iter)
                    )
                    if os.path.exists(tmp_ckpt_path):
                        tmp_opt["path"]["pretrain_model_G"] = tmp_ckpt_path
