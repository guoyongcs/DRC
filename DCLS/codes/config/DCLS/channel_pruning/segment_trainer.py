...
from .segement import DCLSSegment
...
class SegmentWiseTrainer(object):
    ...
    def __init__(self, args, original_model, pruned_model, train_loader, val_loader, prepro, logger, tensorboard_logger, run_count=0):
        self.args = args
        ...
        self.learning_rate = args.opt['train']['lr_G']
        self.scale = args.scale
        self.original_modules = None
        self.pruned_modules = None
        self.run_count = run_count
        
        self.model_segment = DCLSSegment(args, original_model, pruned_model)
            ...
            if isinstance(ori_module, (DPCAB, ResidualBlock_noBN, nn.ModuleList)):
                ...
                if isinstance(ori_module, nn.ModuleList):
                    ...
                    for ori_m, pruned_m in zip(self.original_model.modules(), self.pruned_model.modules()):
                        if isinstance(ori_m, Estimator):
                            ori_module = ori_m
                            pruned_module = pruned_m
                            break
                
                    assert isinstance(ori_module, Estimator)
        ...
        with torch.no_grad():
            for i, test_data in enumerate(self.val_loader):
                ...
                if len(self.args.opt['gpu_ids']) >= 1:
                    lr = test_data["LQ"].cuda()
                    hr = test_data["GT"].cuda()

                sr, _, _ = self.pruned_model(lr)
                # round to uint8
                sr = pruned_utils.tensor2uint(sr) 
                hr = pruned_utils.tensor2uint(hr)
