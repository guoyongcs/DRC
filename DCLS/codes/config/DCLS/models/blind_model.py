...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        self.dual_weight = opt['dual_weight'] if 'dual_weight' in opt else 0
    ...
    def optimize_parameters(self, step):
        sr, kernel, sr2lr = self.netG(self.var_L)
        ...
        if sr2lr is not None:
            dual_loss = self.cri_pix(sr2lr, self.var_L)
            total_loss = total_loss + self.dual_weight * dual_loss
