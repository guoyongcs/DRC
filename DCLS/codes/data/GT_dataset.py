...
class GTDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'data_range' in opt and opt['data_range']:
            start, end = opt['data_range'].split('-')
            start, end = int(start), int(end)
            self.GT_paths = self.GT_paths[start-1:end]
        
        # define the subset of dataset to accelerate the pruning or search process
        if 'subset_ratio' in opt and opt['subset_ratio']:
            data_length = int(len(self.GT_paths) * opt['subset_ratio'])
            self.GT_paths = self.GT_paths[:data_length]
