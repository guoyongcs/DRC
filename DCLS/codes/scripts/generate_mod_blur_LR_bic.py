...
def generate_mod_LR_bic():
    ...
    mod_scale = 4
    # set data dir
    sourcedir = "/mnt/cephfs/home/dengzeshuai/data/sr/benchmark/Set5/HR/"
    savedir = "/mnt/cephfs/home/dengzeshuai/code/DRNv2/temp/setting1/Set5/"
    ...
    for i in range(num_files):
        ...
        for sig in np.linspace(1.8, 3.2, 8):
            ...
            cv2.imwrite(os.path.join(saveLRblurpath, 'sig{:.1f}_{}'.format(sig,filename)), image_LR_blur)
            cv2.imwrite(os.path.join(saveHRpath, 'sig{:.1f}_{}'.format(sig,filename)), image_HR)
            cv2.imwrite(os.path.join(saveKernelblurpath, 'sig{:.1f}_{}'.format(sig,filename)), image_kernel)
        # # LR
