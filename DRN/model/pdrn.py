...
class PDRN(nn.Module):
    def __init__(self, opt, conv=common.default_conv):
        ...
        self.down = [
            common.DownBlock(opt, 2, 
                math.ceil(pruned_feats * pow(2, p)), 
        ...
        up = [[
            common.Upsampler(
                conv, 2, n_feats * pow(2, self.phase), 
                pruned_feats=math.ceil(pruned_feats * pow(2, self.phase)), 
                act=False
            ),
            conv(
                math.ceil(pruned_feats * pow(2, self.phase)), 
        ...
        for p in range(self.phase - 1, 0, -1):
            up.append([
                common.Upsampler(
                    conv, 2, 2 * n_feats * pow(2, p), 
                    pruned_feats=math.ceil(2 * pruned_feats * pow(2, p)),
                    act=False
                ),
                conv(
                    math.ceil(2 * pruned_feats * pow(2, p)), 
                    n_feats * pow(2, p - 1), kernel_size=1
                )
            ])
